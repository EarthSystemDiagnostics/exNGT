...
maxStackedTemperatureNGT <- stackedNGT %>%
  dplyr::mutate(stack = permil2temperature.max * stack)

minStackedTemperatureNGT <- stackedNGT %>%
...
* Recent 11 years versus 1961-1990 reference period:
  ...
  # spatial slope
  (mean(subsetData(stackedTemperatureNGT, t1, var = "stack")) -
   mean(subsetData(stackedTemperatureNGT, t2, var = "stack"))) %>%
    round(digits = 1)

  # minimum slope
  (mean(subsetData(minStackedTemperatureNGT, t1, var = "stack")) -
   mean(subsetData(minStackedTemperatureNGT, t2, var = "stack"))) %>%
    round(digits = 1)

  # maximum slope
  (mean(subsetData(maxStackedTemperatureNGT, t1, var = "stack")) -
   mean(subsetData(maxStackedTemperatureNGT, t2, var = "stack"))) %>%
    round(digits = 1)
