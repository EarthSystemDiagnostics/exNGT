...
Define wrapper function to select certain time periods from data:
```{r}
subsetData <- function(x, t, var) {
  x %>%
    dplyr::filter(Year %in% t) %>%
    dplyr::select(!!as.name(var)) %>%
    unlist()
}
```

## Comparison of NGT temperature stack with Arctic2k

* Overall correlation between NGT temperature stack and Arctic2k compilation
  1000--2000 CE:
  ```{r}
  t <- 1000 : 2000
