...
asp <- 7 / 8.9
w <- 13.6
natfig(file = "./fig/supplement-ngt-arctic2k-running-correlation.pdf",
       height = asp * w, width = w, mfrow = c(2, 1),
...
mtext("a", side = 3, adj = 0.01, padj = 0.5,
      line = -1.2, font = 2, cex = 4 / 3, col = col[1])
mtext("b", side = 3, adj = 0.99, padj = 0.5,
      line = -8.4, font = 2, cex = 4 / 3, col = col[2])

abline(h = 0, lty = 2, lwd = Lwd(1.5), col = "darkgrey")
lines(NGT, type = "l", col = col[1], lwd = Lwd(2.5))
...
axisLwd(4, at = seq(-2, 2, 1), col = col[2], col.axis = col[2])
text(x2, y2, ylab.a2k, srt = -90, xpd = NA, cex = par()$cex.lab * par()$cex,
     col = col[2])

abline(h = 0, lty = 2, lwd = Lwd(1.5), col = "darkgrey")
lines(Arctic2k$Year, Arctic2k$TempAnomaly, col = col[2], lwd = Lwd(2.5))
...
mtext(ylab, side = 2, line = 3.25, cex = par()$cex.lab, las = 0)

mtext("c", side = 3, adj = 0.01, padj = 0.5,
      line = -1.2, font = 2, cex = 4 / 3, col = col[1])

lines(analysis.period, rep(0, length(analysis.period)),
      lty = 2, lwd = Lwd(1.5), col = "darkgrey")

lines(analysis.period, correlation$dat, col = "black", lwd = Lwd(2.5))
lines(analysis.period, correlation$r.upper, lty = 2,
      lwd = Lwd(1), col = "black")
lines(analysis.period, correlation$r.lower, lty = 2,
      lwd = Lwd(1), col = "black")
